...
async function sendIssueMail(subject: string, email: string,body:string, resend: Resend) {
  try {
    ...
    console.log(data, error, "resoo")
    if (error) {
      console.error('Error sending issue mail:', error)
      return { success: false, error }
    }

    console.log(`✅ Issue mail sent successfully from ${email}`)
...
export async function POST(req: NextRequest) {
 ...
 try {
  ...
  const result = await sendIssueMail(subject, email, body, resend)
