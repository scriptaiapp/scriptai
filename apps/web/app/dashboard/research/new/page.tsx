    ...
    const fetchRecentTopics = async () => {
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(error.message || "Failed to fetch recent topics");
        }

        const data = await response.json();
        setRecentTopics(data || []);
      } catch (error: any) {
        toast.error("Error fetching recent topics", {
  ...
  const handleResearchTopic = async () => {
    ...
    try {
      const response = await fetch("/api/research-topic", {
        ...
        body: JSON.stringify({
          topic: autoResearch ? undefined : topic,
          context,
          autoResearch,
        }),
      });
      ...
      const responseRecent = await fetch("/api/research-topic", {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      if (responseRecent.ok) {
        const updatedTopics = await responseRecent.json();
      ...
      toast.error("Error researching topic", {
        description: error.message,
      });
    } finally {
      setLoading(false);
              ...
              <Card>
                ...
                <CardContent className="space-y-6">
                  <div className="space-y-2">
                    <Label htmlFor="topic">Topic (Required unless auto-research is enabled)</Label>
                    <Input
                      id="topic"
                      placeholder="e.g., Artificial Intelligence in Healthcare"
                      value={topic}
                      onChange={(e) => setTopic(e.target.value)}
                      disabled={loading || autoResearch}
                    />
                    <p className="text-sm text-slate-500 dark:text-slate-400">
                      Enter the main topic you want to research or enable auto-research
                    </p>
                  </div>
                  ...
                  <div className="flex items-center space-x-2">
                    <Checkbox
                      ...
                      onCheckedChange={(checked) => setAutoResearch(checked as boolean)}
                      disabled={loading}
                    />
                    <Label htmlFor="auto-research" className="cursor-pointer">
                      Let AI choose a topic based on my channel
                    </Label>
                  </div>
