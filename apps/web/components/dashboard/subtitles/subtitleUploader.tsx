...
export function SubtitleUploader({ onUploadSuccess }: SubtitleUploaderProps) {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            if (!response.ok) throw new Error("Upload failed. Server responded with an error.");
        ...
        <Card className="border-0 shadow-none">
            ...
            <form onSubmit={handleSubmit}>
                <CardContent>
                    <label
                        htmlFor="file-upload"
                        className={`group relative flex w-full cursor-pointer flex-col items-center justify-center space-y-3 rounded-lg border-2 border-dashed p-12 transition-all ${
                            isUploading
                    ...
                    >
                        ...
                        >
                            <UploadCloud className={`h-12 w-12 transition-colors ${
                                file ? "text-purple-600" : "text-gray-400 group-hover:text-purple-500"
                            }`} />
                        </motion.div>
