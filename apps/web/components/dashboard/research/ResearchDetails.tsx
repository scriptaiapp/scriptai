...
export default function ResearchDetails({ loading, research }: ResearchDetailsProps) {
  ...
  const accordionSections = [
    ...
    { id: "sources", title: "Sources", icon: <LinkIcon className="h-5 w-5" />, items: research.research_data.sources, isLink: true },
  ];
          ...
          >
            ...
            >
              <Accordion type="single" collapsible className="w-full space-y-4">
                {accordionSections.map(({ id, title, icon, items, isLink }) => (
                  <motion.div
                    key={id}
                    ...
                    <Card className="border dark:border-slate-800 shadow-sm overflow-hidden">
                      <AccordionItem value={id} className="border-b-0">
                        ...
                        <AccordionContent className="px-4 pb-4">
                          <ul className="space-y-2 pl-6 list-disc marker:text-purple-500">
                            {items.map((item, i) => (
                              <li key={i} className="text-slate-600 dark:text-slate-400 pl-2">
                                {isLink ? (
                                  <a href={item} target="_blank" rel="noopener noreferrer" className="underline hover:text-purple-600 dark:hover:text-purple-400 transition-colors">
                                    {item}
                                  </a>
                                ) : (
                                  item
